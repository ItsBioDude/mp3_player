<script lang="ts">
	...
	// let selectedFolder = '';
	let chosenFolder: string = 'Pops Music';
	...
	if (browser) {
		if (!('chosenFolder' in localStorage) || localStorage.chosenFolder == '') {
			localStorage.setItem('chosenFolder', chosenFolder);
		} else {
			chosenFolder = localStorage.chosenFolder;
		}
	}
	...
	const handleSelectNewFolder = () => {
		fetchData = queryData();
		localStorage.chosenFolder = chosenFolder;
		isNewFolder = true;
	};
</script>
	...
	>
		...
		>
			<select bind:value={chosenFolder} on:change={handleSelectNewFolder}>
				{#each Object.entries(audioData) as [folder]}
					<option value={folder}>
						{folder}
					</option>
				{/each}
